...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['numero_alerta'], $_POST['nuevo_estado'])) {
    $numero = intval($_POST['numero_alerta']);
    $nuevo_estado = intval($_POST['nuevo_estado']);

    $sqlUpdate = "UPDATE Alerta SET ID_Estado = $nuevo_estado WHERE Numero = $numero";
        ...
        <div class="alert-body">
            <div class="alert-text">
                ...
                <p><strong>Estado actual:</strong>
                    <span class="estado-cuadro" id="color-<?php echo $alerta['Numero']; ?>" style="background-color: <?php echo $color; ?>;"></span>
                    <?php echo htmlspecialchars($alerta['EstadoNombre']); ?>
                    <form method="POST" style="display:inline-block;">
                            <input type="hidden" name="numero_alerta" value="<?php echo $alerta['Numero']; ?>">
                        <select name="nuevo_estado" class="form-select form-select-sm estado-alerta d-inline-block" onchange="this.form.submit();" style="width:auto; display:inline-block;">
                            <option value="1" <?php if($alerta['ID_Estado']==1) echo 'selected' ; ?>>Pendiente</option>
                            <option value="2" <?php if($alerta['ID_Estado']==2) echo 'selected'; ?>>En revisión</option>
                            <option value="3" <?php if($alerta['ID_Estado']==3) echo 'selected'; ?>>Aceptada</option>
                            <option value="4" <?php if($alerta['ID_Estado']==4) echo 'selected'; ?>>Rechazada</option>
                            <option value="5" <?php if($alerta['ID_Estado']==5) echo 'selected'; ?>>Finalizada</option>
                        </select>
                    </form></p>
...
<script>
   ...
   const estadoColores = {1:'#ffc400', 2:'#ff8800', 3:'#28a745', 4:'#ff4d4d', 5:'#00ff1a'};

document.querySelectorAll('.estado-alerta').forEach(select => {
    select.addEventListener('change', function() {
        const numero = this.parentElement.querySelector('input[name="numero_alerta"]').value;
        const colorSpan = document.getElementById('color-' + numero);
        if(colorSpan) colorSpan.style.backgroundColor = estadoColores[this.value];
    });
});
